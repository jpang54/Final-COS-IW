...
def get_tweets_predictions(tweets, perform_prints=True):
    ...
    with open("final_tfidf_new.pkl", "rb") as file:
        tf_vectorizer = pickle.load(file)
    with open("final_idf_new.pkl", "rb") as file:
        idf_vector = pickle.load(file)
    with open("final_pos_new.pkl", "rb") as file:
...
if __name__ == '__main__':
    ...
    df2 = df.loc[(df['Number of Likes'] >= 50) | (df['Number of Retweets'] >= 50) | (df['Number of Replies'] >= 50) | (df['Number of Quotes'] >= 50)]
    scraped_tweets = df2.Tweet
    scraped_tweets = [x for x in scraped_tweets if type(x) == str]
    scraped_tweets_predictions = get_tweets_predictions(scraped_tweets)

    print(pd.Series(scraped_tweets_predictions).value_counts())

    df2['class'] = scraped_tweets_predictions
    df2.to_csv(file_name + '-classified50.csv')
